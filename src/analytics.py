...
Computes, for every tracked object on every frame:
  • speed          — instantaneous and rolling-average m/s
  • altitude proxy — coarse estimate from apparent bounding-box size
  • hover state    — sustained low-speed loiter
  • circling state — significant angular displacement over a time window
...
class _TrackState:
    SPEED_WINDOW  = 90    # samples (~3 s at 30 fps) for rolling average
    MAX_POSITIONS = 600   # position history (20 s at 30 fps)

    def __init__(self, track_id: int, centroid: Tuple[int, int],
                 bbox: List[int], timestamp: float):
        ...
        # bbox history for altitude proxy
...
class FlightAnalytics:
    ...
    Configuration keys (all optional, under "analytics.calibration"):
      pixels_per_meter      float  — pixel width of 1 metre at reference distance
      reference_bbox_height float  — bbox height (px) at reference altitude
      reference_altitude_m  float  — real altitude (m) at reference_bbox_height
      hover_speed_mps       float  — m/s below which an object is hovering
      hover_confirm_s       float  — seconds of low speed needed to confirm hover
      circle_window_s       float  — seconds of history used for circle detection
      circle_min_rad        float  — minimum angular displacement (radians) to
                                     flag as circling
    """
    ...
    def update(self, detections: List[dict], timestamp: float) -> Dict[int, dict]:
        """
        Process one frame's detections.  Returns {track_id: analytics_dict}.
        Each analytics_dict contains all metrics for that object this frame.
        """
        ...
        for det in detections:
            ...
            if tid not in self._tracks:
                self._tracks[tid] = _TrackState(tid, (cx, cy), bbox, timestamp)
            ...
            self._update_hover(state, speed, timestamp)
            ...
            result[tid] = {
                ...
                "distance_proxy": round(altitude, 1),   # alias
    ...
    def _update_hover(
        self, state: _TrackState, speed: float, timestamp: float
    ):
    ...
    def _detect_circling(
        ...
        # Need a meaningful radius — ignore tight clusters
